...
def main(train_path, test_path, zip_file_name):
    ...
    vectorizer, cond_prob, priors = train_nb(train_path, model='bernoulli')

    x_train_file_list = get_file_list(train_path)
    x_test_file_list = get_file_list(test_path)

    y_test_true = create_classes(test_path)
    y_test_pred = apply_nb(vectorizer, priors, cond_prob, x_test_file_list)
