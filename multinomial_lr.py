...
def get_file_list(text_file_path): return [os.path.join(text_file_path, file_name)
...
def main(train_path, test_path, zip_file_name):
    ...
    reg_const = learn_reg_const(train_path, model='bow')

    print("Selecting {} as L2 penalty factor in a list of (0.1, 0.5, 1, 5, 10, 50, 100)".format(reg_const))
    print()
    print()

    vectorizer, w = train_lr(x_train_file_list, num_iterations=45, regularization_param=reg_const)
    x_test_file_list = get_file_list(test_path)

    y_test_true = create_classes(x_test_file_list)
    y_test_pred = apply_lr(x_test_file_list, vectorizer, w)
    ...
    print("Stats for Multinomial Logistic Regression...")
